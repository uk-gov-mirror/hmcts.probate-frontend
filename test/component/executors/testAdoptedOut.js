...
describe('coapplicant-adopted-out', () => {
    let testWrapper, sessionData;
    const expectedNextUrlForCoApplicantParentAdoptedIn = CoApplicantParentAdoptedIn.getUrl(2);
    const expectedNextUrlForCoApplicantEmail = CoApplicantEmail.getUrl(1);
    const expectedNextUrlForStopPage = StopPage.getUrl('coApplicantAdoptedOutStop');

    beforeEach(() => {
        testWrapper = new TestWrapper('CoApplicantAdoptedOut');
        sessionData = {
    ...
    describe('Verify Content, Errors and Redirection', () => {
        testCommonContent.runTest('CoApplicantAdoptedOut', null, null, [],
            false, {caseType: caseTypes.INTESTACY}, CoApplicantAdoptedOut.getUrl(1));
        ...
        it(`test it redirects to CoApplicant Adoption place page if child is adopted out: ${expectedNextUrlForCoApplicantParentAdoptedIn}`, (done) => {
            testWrapper.pageUrl = CoApplicantAdoptedOut.getUrl(2);
            testWrapper.agent.post('/prepare-session/form')
                .send(sessionData)
                .end(() => {
                    const data = {
